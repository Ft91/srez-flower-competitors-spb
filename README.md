...
Приложение помогает подготовить закупку цветов и сравнить условия поставщиков. Есть веб-интерфейс на FastAPI и настольная версия Windows на PyQt6 + Qt WebEngine. Выводы модели и актуальность цен требуют проверки.


[Инструкция](docs/INTERFACE.md) · [Windows-приложение](docs/DESKTOP.md) · [Проверки](docs/VALIDATION.md) · [О проекте](CASE_STUDY.md)
...
Материалы собирались 18–19 сентября 2026 года. В [data/](data/README.md) опубликованы первые экраны 7ЦВЕТОВ и ФлораДомСПБ. Реестр sources.json содержит адреса, время сбора и контрольные суммы для всех пяти поставщиков; остальные три изображения сохранены в локальном комплекте. Полный локальный комплект этапа 2 содержит 15 текстовых страниц, 20 скриншотов, два PDF и два XLSX.
...
- Демонстрационный пример без новых обращений к модели или сайтам.
...
Откройте http://127.0.0.1:8000 и нажмите «Демонстрационный пример». Он работает без ключа. Если .env уже существует, не перезаписывайте его. Повторный запуск на подготовленном компьютере: `./start.ps1`.
...
| examples/stage4/ | Три сохранённых ответа для демонстрационного режима |
...
Реально проверены анализ текста и изображения, сохранение истории и сбор Selenium. В демонстрационный пример включены два текстовых анализа и один анализ изображения. Полный анализ всех пяти компаний не выполнялся: для демонстрации проекта достаточно сохранённых примеров.
...
Модель может ошибаться даже при наличии цитаты. В сохранённом ответе OptFlor неточность явно помечена; пользователь должен сверить вывод с исходником. Подробнее: [формат анализа](docs/FORMAT.md).
